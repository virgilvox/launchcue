  ...
  <div class="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full">
      <div class="text-center">
        ...
        <h2 class="mt-2 text-lg font-medium text-gray-600 dark:text-gray-400">Email Verification</h2>
      </div>

      <div class="mt-8 bg-white dark:bg-gray-800 py-8 px-4 shadow-md sm:rounded-lg sm:px-10">
        <!-- Loading state -->
        <div v-if="isLoading" class="text-center">
          <span class="inline-block animate-spin h-8 w-8 border-4 border-primary-600 border-t-transparent rounded-full"></span>
          <p class="mt-4 text-sm text-gray-500 dark:text-gray-400">Verifying your email...</p>
        </div>
        ...
        <!-- Error state -->
        <div v-else-if="error" class="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-900 rounded-md p-4">
          <div class="flex">
            <div class="flex-shrink-0">
        ...
        <div v-else class="text-center">
          <p class="text-sm text-gray-500 dark:text-gray-400">No verification token provided.</p>
        </div>

        <div class="mt-6">
          <div class="text-sm text-center">
            <router-link to="/login" class="font-medium text-primary-600 hover:text-primary-500 dark:text-primary-400 dark:hover:text-primary-300">Go to sign in</router-link>
          </div>
        </div>
      </div>
    </div>
